...
def rec_full(h5fname, rot_center, algorithm, binning, options, num_iter, dark_file, dtype, s_):
    ...
    for iChunk in range(0,chunks):
        ...
        print("Saving Reconstructed Chunk {} of {} to {}.. ".format(iChunk, chunks, fname))
        t0 = time.time()
        ...
        if dtype == 'uint16':
            rec = rec - rec.min()
            rec *= 65535/rec.max()
            dxchange.write_tiff_stack(np.asarray(rec, dtype=np.uint16), fname=fname, start=strt)
        else: 
            dxchange.write_tiff_stack(rec, fname=fname, start=strt, dtype=dtype)
...
def rec(fname, rot_center=0, binning=0, algorithm='gridrec', rec_type='slice',
    ...
    print('All Done ({:.0f} minutes)'.format((time.time() - t0)/60))
