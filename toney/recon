...
def rec_full(variableDict):
    ...
    nSino_per_chunk = 128 # always power of 2               # number of sinogram chunks to reconstruct
    ...
    for iChunk in range(0,chunks):
        ...
        if(iChunk == chunks-1):
                rec = rec[:,data_shape[1]-chunks*nSino_per_chunk]
        dxchange.write_tiff_stack(rec, fname=fname, start=strt)
        strt += int((sino[1] - sino[0]) / np.power(2, float(variableDict['binning'])))

    rec_log_msg = "\n" + "python rec.py --axis " + str(variableDict['rot_center']) + " --type full " + variableDict['fname']
...
def phase_alpha_test(variableDict):
...
def rec_try(variableDict):
    ...
    fname = variableDict['rec_dir'] + os.sep + 'try_rec/' + path_base_name(variableDict['fname']) + os.sep + 'recon_' ##+ os.path.splitext(os.path.basename(variableDict['fname']))[0]    
    for axis in np.arange(*center_range):
...
def main(arg):
    ...
    parser.add_argument("--type", nargs='?', type=str, default="slice", help="Reconstruction type: full, slice, try, phase (default slice). phase tests phase retrieval alpha parameter)")
