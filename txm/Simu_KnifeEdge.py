...
filter_struct = 3 # for the knife edge filtering
#-----------------


####### Replace knife edge acquisitoin
...
####### Treatment after knife edge acquisitoin
# filtering
...
y = deriv_int
amp = np.max(y)
popt = curve_fit(gaus,x,y,p0=[amp,mean_x,sigma]) # gaussian fit on the derivative of raw data
param = popt[0]
sigma1 = param[2] # standard deviation
FWHM1 = 2.3548*sigma1 # FWHM
deriv_int_fit = gaus(x_HR, param[0], param[1], param[2])

y = deriv_int_filt
amp = np.max(y)
popt = curve_fit(gaus,x,y,p0=[amp,mean_x,sigma]) # gaussian fit on the derivative of filtered raw data
param = popt[0]
sigma2 = param[2] # standard deviation
FWHM2 = 2.3548*sigma2 # FWHM
...
TheTitle = 'Knife edge derivative; FWHM: %0.2f' % FWHM1
...
TheTitle = 'Filtered Knife edge derivative; FWHM: %0.2f' % FWHM2
plt.title(TheTitle), plt.xlabel('motor position'), plt.ylabel('Intensity'), plt.grid()
