...
def rec_full(h5fname, rot_center, algorithm, binning):
    ...
    for iChunk in range(0,chunks):
        ...
        # Reconstruct.
        for k in range(4,5):# is optimal
            alpha = 2**k*1e-5
            rec = reconstruct(h5fname, sino, rot_center, binning, alpha, algorithm)
                        # Write data as stack of TIFs.
            fname = os.path.dirname(h5fname) + '/' + os.path.splitext(os.path.basename(h5fname))[0]+ '_full_rec/' + 'recon_' + str(k)
