...
def rec_slice(h5fname, nsino, rot_center, algorithm, binning):
    ...
    for k in range(-10,10):
       rec = reconstruct(h5fname, sino, int(rot_center+k), binning, algorithm)
       fname = os.path.dirname(h5fname) + '/' + 'slice_center_rec/' + 'recon_' + os.path.splitext(os.path.basename(h5fname))[0] + '_' + str(int(rot_center+k)) 
